...
### 📌 성과 30: 2-Phase Loading, TanStack Query 캐시 시딩 및 메모리 룩업, Gzip 압축을 활용한 개념 학습 대시보드 초기 렌더링 및 클릭 반응 속도 극대화 (최대 5초 대기 ➔ 진입 200ms 및 클릭 0ms 단축)
*   **상황 (Issue)**: 핸드북 탭 진입 시 443개 질문의 대용량 데이터(상세 설명, 주의사항, 꼬리 질문, 참고 문헌 포함 약 2.5MB)를 단일 API 호출로 한 번에 조회함에 따라 네트워크 병목 및 브라우저 파싱 부하로 초기 화면 렌더링에 약 3~5초의 극심한 사용자 지연이 발생했습니다. 또한 2-Phase Loading 도입 초기에는 질문 클릭 시마다 상세 내용을 얻기 위해 개별 API 요청을 전개해 매번 뼈대(Skeleton) UI가 반복 노출되어 클릭 경험이 부드럽지 못하고 답답한 사용자 경험 결함이 남았습니다.
*   **해결 방안 (Action)**:
    1.  **2-Phase Loading 파이프라인 설계**: 목록 렌더링에 필수적인 메타데이터만 포함하는 경량 DTO(`QuestionSummaryDto`, 전체 ~100KB) 및 전용 목록 조회 API(`/api/questions/list`)를 신설하여 1차 로드 속도를 획기적으로 개선하고, 상세 설명(Perfect Answer, Caveats 등)은 백그라운드 캐싱 루프를 돌림과 동시에 사용자가 질문을 직접 클릭할 때 동적으로 API(`/api/questions/{id}`)를 통해 지연 로드(On-demand Fetching)하도록 설계했습니다.
    2.  **클라이언트 캐시 동적 주입 및 메모리 단일 룩업 (Cache Seeding & Sync Lookup)**: 백그라운드에서 전체 상세 목록(`ALL_FULL`)을 패치 완료하는 즉시 TanStack Query(React Query) 캐시 저장소에 개별 질문 식별자 키(`["questionDetail", id]`)로 데이터를 사전 분할 주입(Seeding)했습니다. 동시에 활성 질문 결정 시 로컬 `allQuestions` 배열에서 동기식 검색을 우선 적용하여, 백그라운드 로드가 완료된 시점 이후의 클릭 상호작용은 백엔드 API 호출을 전면 배제하고 메모리에서 0ms 내로 즉시 렌더링되게 개선했습니다.
    3.  **JVM 레벨 서버 캐싱 도입**: `QuestionService`에 스프링 `@Cacheable` 애노테이션을 적용하고 JVM 인메모리 캐시를 구성하여 반복적인 데이터베이스(Supabase) I/O 조회 부하를 차단하고 0ms대의 응답력을 갖췄습니다.
    4.  **네트워크 페이로드 Gzip 압축 활성화**: `application.yml` 설정을 통해 응답의 Gzip 압축을 가동(2.5MB ➔ ~500KB로 압축 전송)하여 원격 데이터 서빙 대역폭 점유를 차단했습니다.
    5.  **UX 스켈레톤 로더 기동**: 상세 가이드라인 데이터가 백그라운드에서 완전히 완성되기 전 또는 개별로 지연 로딩되는 찰나의 시점에 사용자 이탈감을 줄이고 웹 접근성(a11y) 기준을 충족하기 위해 단조롭고 차분한 `Skeleton` `animate-pulse` 컴포넌트를 구성해 UI 완성도를 높였습니다.
*   **결과 (Result)**: 초기 화면 목록 표시 속도를 **평균 3~5초에서 200~300ms 내외로 단축(약 90% 성능 향상)**시켰으며, 백그라운드 로드 이후의 모든 개별 질문 상세 조회 지연을 **완벽한 0ms(Instant rendering)**로 튜닝하여 답답함이 없는 매끄러운 탭 및 질문 전환 경험을 확보했습니다.
...
- **모범 답변**:
  - **문제 인식**: 440여 개가 넘는 면접 질문 풀의 전체 원본 데이터(상세 설명, 모범 답안, 주의사항 등)를 첫 화면 진입 시 한 번에 가져오면 페이로드 크기가 약 2.5MB에 달해, 대역폭 부족 상황이나 사양이 낮은 클라이언트 기기에서 네트워크 로딩과 JSON 파싱 비용이 누적되어 화면이 길게 멈추는 현상이 나타났습니다. 또한, 질문 클릭 시마다 API를 매번 다녀오면 사용자에게 지연(Skeleton UI)이 누적되는 한계가 존재합니다.
  - **대안 분석 & Trade-off**:
  ...
  - **최종 설계 합리화**:
    - 클라이언트 사이드 글로벌 검색 기능의 완결성을 보존하면서 극단적인 로딩 가속과 쾌적한 반응 속도를 만족시키기 위해 **하이브리드 2-Phase Loading 및 캐시 레이어(대안 B)**를 최종 적용했습니다. 메타데이터만 묶어놓은 경량 DTO `/api/questions/list` 엔드포인트를 구축하여 진입 즉시 200ms대 내로 탭 목록 화면을 그리게 하였고, 사용자가 클릭하기 전까지는 무거운 데이터 전송을 이연(Lazy loading)했습니다. 특히, 백그라운드에서 전체 데이터 조회가 끝나는 즉시 **TanStack Query의 캐시 데이터셋에 개별 상세 정보를 강제로 사전 주입(Cache Seeding)하고 로컬 State 배열의 동기식 메모리 검색(Preloaded Lookup)**을 동시 가동했습니다. 이 구조 덕분에 백그라운드 로딩 완료 이전에는 접근성 스켈레톤 로더와 개별 API fetch로 부드럽게 방어하고, 로딩 완료 이후에는 네트워크 I/O가 전혀 없는 0ms의 즉시 렌더링 성능을 실현하여 사용자 피로감을 완벽히 해결했습니다. 추가로, 서버 DB 부하 차단을 위해 스프링 `@Cacheable` 기반 JVM 캐시를 구성하고 HTTP 응답 Gzip 압축(압축률 ~80%)을 병행 가동하여 최적의 대용량 웹 리소스 서빙 파이프라인을 구축했습니다.

---

### 📌 성과 31: 데일리 챌린지 구독 메일 카테고리 필터링 매칭 결함 해결 및 로컬 테스트 메일 발송 격리(Mock 튜닝)
*   **상황 (Issue)**: 데일리 챌린지 구독 시스템에서 두 가지 중대한 결함이 식별되었습니다.
    1.  **카테고리 필터 미스매치**: 구독자가 특정 광범위한 범주(예: `Frontend`)를 구독하면, `DailyQuestionScheduler.java`가 질문의 `subject` 필드만 비교하여 매칭 질문을 찾지 못하고 빈 리스트가 되어, 폴백(Fallback)으로 전체 질문 중 무작위 1개를 발송하는 결함 발생(Frontend 구독자에게 CS/DevOps 질문 발송).
    2.  **로컬 테스트 메일 스팸**: 로컬 빌드 및 CI 검증 시 `EmailServiceIntegrationTest.java`가 실행되면서 로컬 `.env`에 정의된 실제 SMTP 설정을 타고 개발자 이메일 계정(`rlawnsgk0610@gmail.com`)으로 지속적인 테스트용 실메일 스팸이 발송되는 결함 발생.
*   **해결 방안 (Action)**:
    1.  **다차원 카테고리 필터링**: `DailyQuestionScheduler.java` 내의 스트림 필터 로직을 수정하여 질문의 구체적인 `subject`뿐만 아니라 광범위한 `category` 필드까지 동시에 비교 검증하도록 보완.
    2.  **테스트용 메일 발송 차단 및 Mocking**: `EmailServiceIntegrationTest.java`에 `@MockBean JavaMailSender`를 구성하여 실제 SMTP 서버 연결을 원천 차단하고, `mailSender.createMimeMessage()` 호출 시 NullPointerException 방지를 위해 dummy `MimeMessage`를 리턴하도록 Stubbing한 뒤, `send` 메서드가 정상 호출되었는지를 `verify`하도록 통합 테스트 고도화.
*   **결과 (Result)**:
    1.  Legacy 및 신규 카테고리(Frontend, Backend 등) 구독 필터링 오차를 0%로 정상화하여 **구독자가 선택한 관심 과목의 질문만 매칭 발송하는 정확성 보장**.
    2.  로컬 Gradle 빌드 및 검증 테스트 시의 **실제 메일 스팸 발송 차단율 100% 달성** 및 빌드 속도 향상.

---

### **Q55. 스프링 배치 스케줄러 내의 카테고리/과목 필터링 매칭 불일치 문제와 로컬 통합 테스트 실행 시 실제 메일 발송망(SMTP)으로 테스트 메일이 발송되는 부작용을 해결한 아키텍처적 및 테스트 대역 설계 전략은 무엇인가요?**
- **모범 답변**:
  - **문제 인식**: 
    1. 구독자가 선택하는 카테고리 목록(예: `Frontend`)은 질문 테이블의 `category` 필드와 매핑되어야 하나, 스케줄러가 질문의 세부 기술 스택인 `subject` 필드만을 기준으로 필터링을 수행하여 매칭에 실패하고 결국 전체 질문을 발송하게 되는 로직 불일치가 있었습니다.
    2. 로컬 개발 환경에서 `.env`에 실제 운영 SMTP 계정 정보가 바인딩된 상태에서 `@SpringBootTest` 통합 테스트가 실행될 때마다, `EmailService`가 SMTP 서버로 직접 메일을 밀어 넣어 실 배달이 발생하는 리소스 낭비 및 스팸 부작용이 잔존했습니다.
  - **대안 분석 & Trade-off**:
    - *카테고리 매칭 필터링*:
      - **대안 A (구독 테이블의 데이터 마이그레이션)**: 구독자의 category 문자열 데이터를 subject 명칭 목록으로 쪼개어 치환. ➔ 기가입 구독자 데이터 구조를 전면 수정해야 하며, 세부 과목이 늘어날 때마다 지속적인 마이그레이션 오버헤드가 발생합니다.
      - **대안 B (스케줄러 내 dual-matching 로직 이식 - 채택)**: 스케줄러의 filter 스트림 내에서 질문의 `subject`뿐만 아니라 `category` 필드도 교차 검사하여 broad 카테고리와 specific 과목 필터링을 동시 지원하게 설계합니다.
    - *이메일 테스트 스팸 차단*:
      - **대안 1 (테스트 환경 SMTP 비활성화)**: `test` 프로필의 application.yml에서 메일 설정을 지우거나 가짜 호스트로 바인딩. ➔ `JavaMailSender` 빈 초기화 자체가 누락되어 `EmailService` 생성 시 빈 누락으로 스프링 컨텍스트 로드가 실패하는 빌드 크래시를 낳습니다.
      - **대안 2 (@MockBean을 통한 JavaMailSender 격리 - 채택)**: Spring Test 프레임워크의 `@MockBean`을 통해 `JavaMailSender`를 Mock 객체로 컨텍스트에 교체 주입합니다. 단, 비동기 발송 로직 내 `createMimeMessage()`가 null을 리턴해 NPE가 발생하는 문제를 방어하기 위해 dummy `MimeMessage`를 Stubbing하고 최종 메일 발송 요청(`send`) 행위만 검증합니다.
  - **최종 설계 합리화**:
    - 시스템 정합성 보존과 클린 빌드 DX(Developer Experience)를 위해 **dual-matching 필터 설계(대안 B)** 및 **@MockBean Mocking & Stubbing 통합 테스트 전략(대안 2)**을 최종 적용했습니다. 스케줄러의 질문 조회 스트림 필터링을 고도화하여 `category`와 `subject`를 교차 검사함으로써, Frontend/Backend 등의 대범주 구독자와 javascript/java 등의 세부 도메인 구독자 모두에게 단 한 건의 오발송 없이 질문이 정밀 매칭되도록 완성했습니다. 또한 테스트 시 `@MockBean JavaMailSender`를 선언하고 `jakarta.mail.Session`을 null로 주입한 dummy `MimeMessage`를 모킹 및 반환하도록 튜닝하여, 실제 SMTP 연결 비용을 0%로 줄여 테스트 실행 속도를 극대화하면서도 외부 메일함 스팸 전송을 완벽히 격리 차단했습니다.
