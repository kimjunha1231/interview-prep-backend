...
## 🎨 15. WAI-ARIA 웹 접근성(a11y) 가이드라인
*   **시맨틱 마크업**: 단순 목록이나 레이아웃 구분에 `div`를 남발하지 않고 `<ul>`/`<li>`, `<nav>`와 같은 표준 시맨틱 요소를 적극 채용하여 보조 기기(스크린 리더)의 구조 해석 직관성을 확보합니다.
*   **아코디언 및 대화형 컴포넌트**: 키보드로 초점을 받을 수 없는 `div` 클릭 핸들러 대신 `<button type="button">`을 적극 활용하고, `aria-expanded` (열림 여부), `aria-controls` (제어하는 패널 ID)를 상호 바인딩하여 상태 변화를 리더기에 전파합니다. 또한 내부 패널에는 `role="region"` 및 `aria-labelledby`를 설정하여 영역 정체성을 보장합니다.
*   **스크린 리더 전용 라벨 (`sr-only`)**: 디자인 시안에 라벨 텍스트가 표시되지 않는 미니멀 폼(예: 이메일 입력, 카테고리 선택)의 경우, 화면에는 보이지 않지만 스크린 리더에는 정상적으로 포커스 정보가 읽히도록 `sr-only` CSS 클래스를 적용한 `<label htmlFor="...">`를 반드시 짝지어 배치합니다.
*   **비동기 알림 (`aria-busy` & `aria-live`)**: 스켈레톤 로더나 데이터 동적 수신 영역에는 `aria-busy="true"` 및 `aria-live="polite"`를 선언하여 시각적으로 대기 중인 상태를 보조기기 사용자도 부드럽게 인지하도록 유도합니다.

---

## 📊 16. Vercel Web Analytics & 동적 Sitemap 컴파일 파이프라인
*   **초경량 웹 애널리틱스 연동**: 초기 웹 성능(Core Web Vitals)의 점수 보전을 위해 1KB 미만의 경량 풋프린트를 가진 `@vercel/analytics`를 채택, `src/main.tsx` 루트 단에서 `<Analytics />` 컴포넌트를 마운트하여 실시간 트래픽을 비침습적으로 캡처합니다.
*   **빌드타임 Sitemap 자동 갱신 및 2-Stage 폴백 엔진**: 440개가 넘는 dynamic 질문 정보(`/` 및 `/?questionId=X`)를 검색엔진 및 AI 봇(GEO)이 크롤링할 수 있도록 Node.js 기반 `generate-sitemap.js`를 구축하고 `prebuild` 및 `build` 체인에 연동했습니다. 로컬 빌드 시에는 백엔드의 `questions.json` 원본을 파싱하여 sitemap을 빌드하는 동시에 질문의 총 개수 요약 정보인 `questions-metadata.json` (18바이트)을 자동 갱신하고, Vercel 클라우드의 격리 빌드 환경(상위 백엔드 디렉토리 미존재)에서는 이 메타데이터 캐시 파일을 폴백(Fallback)으로 자동 스위칭하여 빌드 크래시를 방지합니다.

---

## ⚡ 17. 대용량 데이터 로딩 개선을 위한 2-Phase Loading 및 다층 캐싱/Gzip 최적화
