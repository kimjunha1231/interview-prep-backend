...
public class InterviewControllerTest {
    ...
    @Test
    @DisplayName("POST /api/interviews/sessions/{sessionId}/answers API 요청 시 답변을 제출하고 피드백 결과를 반환한다")
    void testSubmitAnswerApi() throws Exception {
        ...
        given(interviewService.submitAnswer(eq("test-uuid-key"), eq(1L), eq("My Answer")))
                .willReturn(history);
        ...
        // When & Then
        mockMvc.perform(post("/api/interviews/sessions/test-uuid-key/answers")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(requestBody)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    @Test
    @DisplayName("GET /api/interviews/sessions/{sessionId}/history API 요청 시 특정 세션의 면접 보고서 이력을 조회한다")
    void testGetSessionHistoryApi() throws Exception {
        ...
        given(interviewService.getSessionHistory("test-uuid-key"))
                .willReturn(List.of(history));

        // When & Then
        mockMvc.perform(get("/api/interviews/sessions/test-uuid-key/history"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
